...
class Chat(Plugin):
    ...
    def on_text_command(self, event, members):
        if event.guild in [
                member for member in [conv.members for conv in CONVERSATIONS]
        ]:
        ...
        with TinyDB('phonebot.json') as db:
            for member in members.split(' '):
                if member not in db.get(Query().guild_id == int(
                        event.guild.id))['friendlist'].keys():
                    event.msg.reply("You are not friends with this server...")
                    return

                snowflake = db.get(Query().guild_id == int(
                    event.guild.id))['friendlist'][member]
